 ...
 */
public class ProjectileLightAntimatterMissile extends Projectile {
    ...
    public ProjectileLightAntimatterMissile(ProjectileType type, Enemy target, float x, float y, int width, int height) {
    ...
    public void update() {
        ...
        if (this.getSpeed() < 1000 && this.projectileFlightTime > (this.projectileLastAccelerationTime + 50)) {
            this.projectileLastAccelerationTime = this.projectileFlightTime;
            this.setSpeed(this.getSpeed() * (float) 1.02);
        }
    }
}